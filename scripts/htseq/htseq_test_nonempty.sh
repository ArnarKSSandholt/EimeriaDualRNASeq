...
do
    ...
    htseq-count -f bam -r pos -s reverse -i Parent -m intersection-nonempty ${f}_Aligned.sortedByCoord.out.bam results/star/merged_reference/eimeria_chicken_merge.gff \
        > results/htseq/test/${GROUP_NAME}/${OUTPUT_NAME}_test-nonempty.counts
done